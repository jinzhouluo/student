...
class AppiumTests(ParametrizedTestCase):
    def setUp(self):
...
fruitd = [[{'port':'4725','deviceName':'192.168.0.211:5555'}],[{'port':'4723','deviceName':'192.168.0.203:5555'}]]
def f():
    pool = Pool(2)
    pool.map(sample_request, fruitd)
    pool.close()
    pool.join()

def sample_request(device):
    suite = unittest.TestSuite()
    suite.addTest(ParametrizedTestCase.parametrize(AppiumTests,param=device ))    
...
if __name__ == "__main__":
    ...
    #  suite = unittest.TestSuite()
    #  suite.addTest(ParametrizedTestCase.parametrize(AppiumTests, param=fruitd[i][1],params=fruitd[i][0]))  
